...
Ext.onReady(function() {
    ...
    taskTreeStore.getProxy().extraParams = { user_id: @current_user_id@, format: 'json' };
    taskTreeStore.load({
    ...
    // Load stores that need parameters
    hourIntervalStore.getProxy().extraParams = { user_id: @current_user_id@, format: 'json' };
    hourIntervalStore.load({
        callback: function() {
            console.log('PO.store.timesheet.HourIntervalStore: loaded');
        }
